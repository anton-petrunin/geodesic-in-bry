...
\begin{thm}{Minor theorem}\label{cor:main}
...
A proof of the minor theorem in case $\dim M=3$  has been sketched by Robert Bryant \cite{Bryant};
for $\dim M\ge 4$ it was proved by Thomas Murphy and Frederick Wilhelm \cite{Wilhelm}.
...
In the case that the  set $Q$ consist of $3$ points, this corollary solves the \emph{standing conjecture} formulated by Marcel Berger \cite[Note 6.1.3.1]{berger-2003}.
It seems plausible that  the following more exact form of the conjecture might be squeezed out from our key lemma.
...
More open questions are listed in  Appendix \ref{app:remarks}.
...
If $d=m$, then by the invariance of domain we have  $\mathfrak C\setminus \mathfrak B$ is open in $M$; that is, $\mathfrak C$ is full-dimensional --- the minor theorem is proved.
