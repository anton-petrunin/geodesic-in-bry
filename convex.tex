...
Let $\mathfrak C$ be an arbitrary closed convex subset of a $\mathcal C^2$-generic Riemannian manifold $(M,g)$.
...
or \emph{full-dimensional}; that is, the the interior of $\mathfrak C$ is nonempty.
\end{thm}
...
Let $Q$ be a closed subset of a $\mathcal C^2$-generic Riemannian manifold $(M,g)$ of dimension at least $3$. If $Q$ does not lie in a minimizing geodesic and
...
Its formulation uses the notion of \emph{rank} of a point $p$ in a convex set $\mathfrak{C}$;
...
In Appendix \ref{app:remarks} we list a few open questions.
...
We define the \emph{rank} of $x$ in $\mathfrak{C}$ as the dimension of $\L_x\mathfrak{C}$.
...
we set $R^1=0$.
Note that $R^k_\vec x\:\T_p\z\to \T_p$ is a self-adjoint operator and $\vec x\mapsto R^k_\vec x$ is a homogeneous polynomial of degree $k$ and 
...
If $d=1$, then $\mathfrak C\setminus \mathfrak B$ is a minimizing geodesic in $(M,g)$;
hence $\mathfrak C$ is contained in a minimizing geodesic as well.
If $d=m$, then $\mathfrak C\setminus \mathfrak B$ is open in $M$; that is, $\mathfrak C$ is full-dimensional.
...
Recall that $k$-jet of $G$ can be thought of as the Taylor polynomial of $G$ of degree $k$
...
Note that $G^1=0$; equivalently christoffels vanish in the normal coordinates.
...
The codimension of this subspace in $\mathcal{S}^{k-1}_\vec x$ grows with $k$.
