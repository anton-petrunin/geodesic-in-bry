...
As a corollary we obtain a peculiar properties of \emph{all} convex sets that hold true in any \emph{generic} Riemannian manifold $(M,g)$.
...
It agrees with general pattern: \emph{a random object in your favorite theory  looks like nothing you have ever seen before}.
...
In particular, if $\dim M\ge 3$ and $\mathfrak{C}$ is bounded by a smooth hypersurface, then it is strictly convex; that is interior points of any geodesic in $\mathfrak{C}$ lie in the interior of $\mathfrak{C}$.
\end{thm}

A proof of a weaker version of the theorem was sketched  the second author \cite{petrunin-2009}.

The proof of proposition is built on the key lemma stated in the following section; 
...
The remaining part is straightforward: we apply the Thom transversality theorem to show that the curvature tensor of a generic Riemannian manifold does not meet this property.
...
Note that the $\K_x\mathfrak{C}$ is a convex cone in $\T_x$ and it coincides with 
$\T_x$ if and only if
$x$ lies in the interior of $\mathfrak{C}$. %???REF, see, for instance, \cite{}.

Given $x\in \mathfrak{C}$, denote by $\L_x\mathfrak{C}$ the maximal linear subspace of $\K_x\mathfrak{C}$.
Recall that $\dim \L_x\mathfrak{C}$ is the rank of $\mathfrak{C}$ at $x$.
...
Suppose a nonconstant geodesic $\gamma$ in a Riemannian manifold has a parallel family of subspace $V_t \subset \T_{\gamma (t)}$ that are nontrivila invariant subspaces of $R(\gamma'(t))$ for any $t$.
In this case we say briefly that $\gamma$ admits a \emph{ invariant family of subspaces}.

Let $M$ be an $m$-dimensional smooth manifold.
Choose a compact subset $K\subset M$ and $\eps>0$.
Consider the set $G(K,\eps)$ of all Riemannian metrics $g$ on $M$ such that there exists a nonconstant geodesic $\gamma$ in $(M,g)$ with a nontrivial parallel invariant family of subspaces.
...
Note that $g\in G(M)$ if and only if $(M,g)$ has a nonconstant geodesic with a nontrivial parallel invariant family of subspaces.
...
then $(M,g)$ does not have nonconstant geodesic with a nontrivial parallel invariant family of subspaces.
\end{thm}
...
Let $\mathfrak{C}$ be a closed convex set in a Riemannian manifold $(M,g)$.
Then the set of points in $\mathfrak{C}$ with rank $\le k$ is countably \emph{$k$-rectifiable};
...
By Proposition \ref{prom:rank}, the tangent cone $\K_{\gamma (t)} \mathfrak C$ is either the whole tangent space $\T_{\gamma (t)}$ or it does not contain any plane.
...
Thus we have subdivided the set of non-extreme points of $\mathfrak C$ into $(M,g)$-geodesics with positive lengths.
...
It worth to mention the following closely related result of Anatoliy Milka \cite[§~4]{milka}; originally it was proved for Euclidean space, but the same proof works for Riemannian manifolds:
