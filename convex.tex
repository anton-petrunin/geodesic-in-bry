...
Then any point of rank $\leq 1$ of $\mathfrak {C}$ lie on minimizing geodesics between points in $Q$.
\end{thm}


Together with the Main Proposition it would imply that in a generic Riemannian manifold of dimension at least 3
the closure of the convex hull of any $\mathcal C^1$-submanifold a $\mathcal C^1$-boundary.  On the contrary, 
 in the Euclidean space   the structure of singularities of convex hulls of smooth submanifolds is highly non-trivial, compare,
\cite{Sedykh}.
...
There is a hope that the presented argument, when properly extended to infinite-dimensional manifolds, might lead to a negative answer to the following question of Mikhael Gromov \cite[6.B\textsubscript{1}(f)]{gromov-1993}.

\begin{thm}{Question}
Let $X$ be a complete  $\CAT(0)$ space (not necessary locally compact).
Is it true that any compact set of $X$ lies in a compact convex subset?
\end{thm}

The fact that convex sets and their extremal points in CAT(0) spaces behave  differently as in the Riemannian
world is  supported by \cite{Monod}.
