...
As a corollary we obtain a peculiar property of \emph{all} convex sets that hold true in a \emph{generic} Riemannian manifold $(M,g)$.
Namely, we show that the set of non-extreme points on the boundary of any convex set in $(M,g)$
is a union of a countable set of geodesics.
\end{abstract}

\section{Introduction}
Let $\mathfrak{C}$ be the convex hull of a subset $Q$ in a Euclidean space.
By Carathéodory's theorem, any point in $\mathfrak{C}\setminus Q$ is an inner point of a geodesic contained in $\mathfrak{C}$;
...
This statement is also easily seen to hold locally in any two-dimensional Riemannian manifold.
...
Given a positive integer $k$, we say that a property $P$ holds for \emph{$\mathcal C^k$-generic} Riemannian metric $g$ on a manifold $M$ 
if the property $P$ holds for a dense G-delta set of metric tenors in the $\mathcal C^k$-topology.
...
Then, for $\mathcal C^2$-generic Riemannian metric $g$ on $M$,
...
A proof of a weaker version of the theorem was sketched  the second author \cite{petrunin-2009}.
...
The proofs are built on the key lemma stated in the following section; 
...
If the tangent cone to the convex hypersurface at a point of the geodesic contains a plane, this condition implies a non-trivial
statement on the curvature tensor of the manifold.
The remaining part is straightforward: we show that the curvature tensor of generic Riemannian manifolds do not have this property and apply the Thom transversality theorem.
...
the \emph{tangent cone} $\K_x\mathfrak{C}\subset \T_x$ is defined as the closure of the set of all velocity vectors of geodesics that start at $x$ and run in $\mathfrak{C}$.
Note that the $\K_x\mathfrak{C}$ is a convex cone in $\T_x$ and it coinicdes with 
$\T_x$ if and only if
$x$ lies in the interior of $\mathfrak{C}$. %???REF, see, for instance, \cite{}.
...
By that reason one may regard $R(\vec{x})$ as \emph{Jacobi operator}.

\begin{thm}{Key lemma}\label{lem:key}
Let $(M,g)$ be a Riemannian manifold and $\gamma\:(a_0,b_0)\to M $ be a geodesic contained in a closed convex set $\mathfrak{C}\subset (M,g)$.
...
The maximal linear subspace $L(a)$ of $\K_{\gamma(a)}\mathfrak{C}$ is an invariant subspace of~$R(\gamma'(a))$.
\end{subthm}

\end{thm}

To prove the main part we use the fact that the parallel translation can be defined via geodesics;
...
That is, the tangent cone $\K_{\gamma(t)}\mathfrak{C}$ is parallel along $\gamma$ --- part \ref{SHORT.lem:key:a} is proved.
...
Since $\K$ is a closed convex cone, we get $R(\gamma')\cdot \vec{v}\z\in \K_\vec{v}\K$, proving part \ref{SHORT.lem:key:a}.
...
\K_\vec{v}\K=\K.
\]
Therefore, if $\vec{v}\in L(a)$, then $\pm R(\gamma'(a))\cdot \vec{v}\in \K$ and hence $R(\gamma'(a))\cdot \vec{v}\in L(a)$.
That is, $L(a)$ is an invariant subspace of $R(\gamma'(a))$ --- part \ref{SHORT.lem:key:b} is proved.
\qeds
...
We present calculations since some notations will be used further.
...
Note that by \ref{lem:jacobi:A-R}, the space $\mathcal{R}^{n-2}$ is a linear with respect to standard addition and multiplication by a scalar.
Moreover the map $\rho_n\:\mathcal{J}^n(p)\to \mathcal{R}^{n-2}(p)$ is onto.
...
\section{Higher Jacobi operators}
...
Given an integer $k\ge 0$,
the operator $R^{(k)}(\vec{x})$ has 4 \emph{trivial} invariant subspaces of dimension $0$, $1$ (the subspace spanned by $\vec{x}$), $n-1$ (the orthogonal complement to $\vec{x}$) and $n$;
...
Consider the set $G(K,\eps)$ of all Riemannian metrics $g$ on $M$ satisfying the following property:
there exists a geodesic $\gamma$ in $(M,g)$ of length  $\epsilon$ that starts in $K$ and a parallel family $V_t \subset \T_{\gamma (t)}$ of nontrivial invariant subspaces of $R(\gamma'(t))$.
...
\begin{thm}{Claim}
$G^k(K)\z\subset G(K,\eps)$ for any integer $k\ge 0$, compact set $K\subset M$, and $\eps>0$.
\end{thm}
...
Then, for $k\geq 6$, the set $G^k(K)$ is nowhere dense in the space of all Riemannian metrics.
\end{thm}
...
The latter follows since the self-adjoin operators $R(\vec{x}),...,R^{(k)}(\vec{x})$ can be arbitrary, provided $R^{(i)}(\vec{x})=0$ for any $i$.

Consider the union $\Sigma$ of these subspaces for all unit vectors $\vec{x}\in \T_p$.
Note that $\Sigma\subset \mathcal{R}^k(p)$ is algebraic and its codimension at least 
...
The statements in the previous section imply the following:

\begin{thm}{Claim}\label{clm:meager}
The set $G(M)$ is meager; that is, its complement is a dense G-delta set in the space of all Riemannian metrics on $M$ with $\mathcal{C}^2$ topology.
\end{thm}
...
\parit{Proof.}
Choose normal coordinates $(u_1,\dots, u_m)$ in a small spherica neighborhood of $x$.
...
Let $\mathfrak{C}$ be a closed convex set in a Riemannian manifold.
Then the set $S_k$ of points $x\in \mathfrak{C}$ such that $\K_x\mathfrak{C}$ does not contain a linear subspace of dimension $k+1$ is countably \emph{$k$-rectifiable};
that is, $S_k$ can be  covered by images of a countable set of Lipschitz maps $\RR^k\to \mathfrak{C}$.
\end{thm}

\parit{Proof of the main theorem (\ref{thm:main}).}
By \ref{clm:meager}, generic Riemannian metric $g$ on $M$ does not lie in $G(M)$.
Suppose $\gamma$ is a geodesic in $(M,g)$ that runs in a closed convex set $\mathfrak C$.
According to the key lemma (\ref{lem:key:b}), the tangent cone $\K_{\gamma (t)} \mathfrak C$ is either the whole tangent space $\T_{\gamma (t)}$ or it does not contain any plane.
...
Thus, $d=1$ or $d=m$.
In the first case, $\mathfrak C\setminus \mathfrak B$ is a geodesic in $(M,g)$, hence $\mathfrak C$ is contained in a geodesic as well.
In the latter case, $\mathfrak C\setminus \mathfrak B$ is open in $M$.
This proves~\ref{SHORT.thm:main:interior}.


In order to prove \ref{SHORT.thm:main:geodesics}, we observe that for any non-extreme point $x\in \partial \mathfrak C$,
the tangent cone $\K_x \mathfrak C$ splits off a line but not a plane.
Thus this line is unique and is the tangent line of the (therefore unique) geodesic $\gamma$ contained in $\mathfrak C$ and having $p$ as an inner point. 

Let us extend $\gamma$ to a maximal open interval, so that $\gamma$ stays in $\mathfrak C$.
By the first statement of the key lemma, all points on $\gamma$ lie on $\partial \mathfrak C$.
...
Thus we have subdivided the set of non-extreme points of $\mathfrak C$ into $(M,g)$-geodesics of positive length.
...
We start with $Q=Q_1$ and define inductively $Q_{i+1}$ to be the union of all minimizing geodesics between pairs of points of $Q_i$.
Then the union $\mathfrak{C}= \bigcup_{i} 
Q_i$ is the convex hull of $Q$.


Assume now that $\mathfrak{C}$ is closed and not equal to $M$.
Since $Q$ is not contained in a geodesic, by our main theorem, $\mathfrak{C}$ has a non-trivial interior.
By the explicit construction of $\mathfrak{C}$ above, any point $x\in \mathfrak{C} \setminus Q$ is not an extreme point of $\mathfrak{C}$.
...
But the 
(by assumption non-empty) $(m-1)$-dimensional topological manifold $\partial \mathfrak{C} \setminus Q$ has Hausdorff dimension $m-1$ and cannot be a union of countably many geodesics.
...
On the other hand all examples we see the set of geodesics on convex hypersurfaces in generic manifolds is locally finite.


There is a chance that presented argument might help to construct a counterexample to the following well known conjecture:
