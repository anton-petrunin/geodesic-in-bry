...
Assuming that the answer is affirmative, it can be combined with the main proposition to derive some properties of convex hull in generic Riemannian manifolds.

\begin{thm}{Question}
Let $\mathfrak{C}$ be the closure of a convex hull of a set $Q$ in a Riemannian manifold.
Then all point of $\mathfrak {C}$ with rank at most $1$ lie on minimizing geodesics between points in $Q$.
\end{thm}
...
A surprising behavior of convex sets in complete (but not locally compact) $\CAT(0)$ spaces   discussed by Nicolas Monod \cite{monod}.

Finally let us mention that there is a result of Anatoliy Milka \cite[§~4]{milka} about rank of points on geodesics in intrinsic metric of convex surface; it is closely related to our main proposition, but goes in the opposite direction.
